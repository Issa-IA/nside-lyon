...
class inheritTask(models.Model):
    ...
    sequence1 = fields.Char(string='Dossier N°', required=True, copy=False, readonly=True,
    ...
    def _compute_num_cartons_client(self):
        for task in self:
            task.num_cartons_client = task.carton_ids.search_count([('proprietaire_carton', '=', 'ICR')])
    ...
    def _compute_intervention_unique_ids(self):
        for task in self:
            ...
            task.intervention_unique_ids = [(5, 0, 0)] + [(0, 0, {
                'etiquette_id': line.etiquette_id.id,
                'quantity_ok': line.quantity_ok,
                'quantity': line.quantity,
                'quantity_hs_piles': line.quantity_hs_piles,
                'quantity_hs': line.quantity_hs,
                'quantity_cassees': line.quantity_cassees,
                'quantity_illisible': line.quantity_illisible
            }) for line in unique_intervention_lines]
    ...
    def create_prestation_lines(self):
        ...
        for record in self.intervention_unique_ids:
            ...
            if liste_de_prix:
                product_prices = {(item.product_id.id, item.fixed_price): item for item in liste_de_prix.item_ids}

                for (product_id, price), item in product_prices.items():
                    existing_line = prestation_ids.filtered(
                        lambda line: line.product.id == product_id and line.price == price)
    ...
    def create_sale_order(self):
        ...
        for task in self:
            ...
            task.sale_order_intervention_id = order.id
            ...
            for intervention in task.intervention_unique_ids:
                for component in intervention.etiquette_id.composant_ids:
                    ...
                    quantity = component.quantity * intervention.quantity_ok
            ...
            # Additional lines for carton, product_carton, palette_transport
...
class Carton(models.Model):
    ...
    def write(self, values):
        ...
        new_lines = self.intervention_line_illisible_ids.filtered(
            lambda line: not self.env['intervention.line.eeg'].search([('illisible_id', '=', line.id)])
        )

        for intervention_line in new_lines:
            line_values = {
            ...
            self.env['intervention.line.eeg'].create(line_values)
...
class InterventionLineEeg(models.Model):
    ...
    def create(self, values):
        ...
        if active and self.detect_duplicates(serial_number_36) > 1:
...
class InterventionLineIllisible(models.Model):
    ...
    carton_id = fields.Many2one('carton.carton', 'Carton',
                                default=lambda self: self.env['carton.carton'].search([], limit=1), index=True,
                                copy=False)
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company, required=True)
